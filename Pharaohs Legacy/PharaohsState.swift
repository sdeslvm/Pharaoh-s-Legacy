...
protocol PharaohsLoadStateRepresentable {
    var type: PharaohsLoadState.StateType { get }
...
struct PharaohsLoadState: Equatable, PharaohsLoadStateRepresentable {
    ...
    static func idle() -> PharaohsLoadState {
        PharaohsLoadState(type: .idle, percent: nil, error: nil)
    }

    /// Создание состояния прогресса
    static func progress(_ percent: Double) -> PharaohsLoadState {
        PharaohsLoadState(type: .progress, percent: percent, error: nil)
    }

    /// Создание состояния успеха
    static func success() -> PharaohsLoadState {
        PharaohsLoadState(type: .success, percent: nil, error: nil)
    }

    /// Создание состояния ошибки
    static func error(_ err: String) -> PharaohsLoadState {
        PharaohsLoadState(type: .error, percent: nil, error: err)
    }

    /// Создание состояния отсутствия подключения
    static func offline() -> PharaohsLoadState {
        PharaohsLoadState(type: .offline, percent: nil, error: nil)
    }

    // MARK: - Методы сравнения

    /// Пользовательская реализация сравнения
    func isEqual(to other: PharaohsLoadState) -> Bool {
    ...
    static func == (lhs: PharaohsLoadState, rhs: PharaohsLoadState) -> Bool {
        lhs.isEqual(to: rhs)
    }
}
...
extension PharaohsLoadState: CustomStringConvertible {

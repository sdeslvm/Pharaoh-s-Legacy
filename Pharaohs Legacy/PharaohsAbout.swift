...
struct PharaohsGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://pharlegacy.top/play")! }

    var body: some View {
        ZStack {
            Color(hex: "#000")
                .ignoresSafeArea()
            PharaohsEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}

#Preview {
    PharaohsGameInitialView()
}
